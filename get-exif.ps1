# Get-Exif, version 0.6.6 08.02.2025
...
#               0x9202 ApertureValue     RATIONAL (1)       
        ...
        {
        Write-Error "Failed to get EXIF data: $_"
        return $null
        }
    }
...
Function Main 
    ...
    try 
        ...
        {
        Write-Error "Failed to load required assembly: $_"
        return
        }

    # Let's have a look at what arguments the user provided on the command line
    If ('' -eq $Path) 
        {
        If ($IsWindows) 
            {
    ...
    # Let's check that the the user submitted file exists
    ...
    try 
        ...
        if (Test-Path -Path $Path -PathType Leaf) 
            ...
            Else 
                {
                Write-Error "Failed to retrieve EXIF data."
                }
            } 
        Else 
            {
